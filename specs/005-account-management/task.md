...
- [x] **T-01** Create `AccountStatus` enum with values: `Active`, `Inactive`
  - File: `src/CRM.Domain/Enums/AccountStatus.cs`

- [x] **T-02** Create `Account` entity extending `BaseEntity` with fields: `AccountName`, `Industry`, `Website`, `Phone`, `Status`, `IsDeleted` and navigation property `Contacts` (ICollection<Contact>)
  - File: `src/CRM.Domain/Entities/Account.cs`

- [x] **T-03** Update `Contact` entity — change `AccountId` navigation property reference from `Customer` to `Account`
...
- [x] **T-04** Create `IAccountRepository` interface with methods: `GetByIdAsync`, `GetPagedAsync`, `AddAsync`, `UpdateAsync`, `SoftDeleteAsync`
  - File: `src/CRM.Application/Interfaces/IAccountRepository.cs`

- [x] **T-05** Create `AccountSummaryDto` (Id, AccountName, Industry, Phone) and `AccountDetailDto` (all fields including Status)
  - File: `src/CRM.Application/Features/Accounts/AccountDtos.cs`

- [x] **T-06** Create `CreateAccountCommand`, `CreateAccountValidator`, `CreateAccountHandler`
  - Folder: `src/CRM.Application/Features/Accounts/CreateAccount/`
  - Validator rule: AccountName required

- [x] **T-07** Create `UpdateAccountCommand`, `UpdateAccountValidator`, `UpdateAccountHandler`
  - Folder: `src/CRM.Application/Features/Accounts/UpdateAccount/`
  - Validator rule: same as create

- [x] **T-08** Create `GetAccountByIdQuery` and `GetAccountByIdHandler`
  - Folder: `src/CRM.Application/Features/Accounts/GetAccountById/`
  - Throw `NotFoundException` if account not found

- [x] **T-09** Create `GetAccountsQuery` and `GetAccountsHandler` with pagination and search
  - Folder: `src/CRM.Application/Features/Accounts/GetAccounts/`
  - Query params: `Search`, `Page` (default 1), `PageSize` (default 10)

- [x] **T-10** Create `DeleteAccountCommand` and `DeleteAccountHandler`
...
- [x] **T-11** Create `AccountConfiguration` EF Core entity type configuration
...
- [x] **T-12** Update `ContactConfiguration` — change FK reference from `Customers` to `Accounts` table
  - File: `src/CRM.Infrastructure/Persistence/Configurations/ContactConfiguration.cs`

- [x] **T-13** Add `DbSet<Account> Accounts` to `AppDbContext`
  - File: `src/CRM.Infrastructure/Persistence/AppDbContext.cs`

- [x] **T-14** Implement `AccountRepository` with all `IAccountRepository` methods
...
- [x] **T-15** Register `IAccountRepository` → `AccountRepository` in DI container
  - File: `src/CRM.API/Program.cs`

- [x] **T-16** Add and apply EF Core migration for Account table
  ```
...
- [x] **T-17** Add authorization policies: `accounts.view`, `accounts.create`, `accounts.edit`, `accounts.delete`
  - File: `src/CRM.API/Program.cs`

- [x] **T-18** Create `AccountsController` with 5 endpoints
...
- [x] **T-19** Create accounts API client with types and axios functions
...
- [x] **T-20** Create `AccountListPage` with search, paginated table, add/edit/delete actions
...
- [x] **T-21** Create `AccountFormPage` for create and edit modes
...
- [x] **T-22** Add Account routes to app router
  - File: `web/src/App.tsx`
  - Routes: `/accounts`, `/accounts/new`, `/accounts/:id/edit`

- [x] **T-23** Add "Accounts" nav item to sidebar/navigation
  - File: `web/src/components/Layout/` (sidebar component)

- [x] **T-24** Update Contact form Account dropdown to use `getAccounts` instead of `getCustomers`
...
- [x] **T-25** Write `CreateAccountValidatorTests` — AccountName required rule
  - File: `src/CRM.Tests/Features/Accounts/CreateAccountValidatorTests.cs`

- [x] **T-26** Write `CreateAccountHandlerTests` — entity creation and repository call
  - File: `src/CRM.Tests/Features/Accounts/CreateAccountHandlerTests.cs`

- [x] **T-27** Write `GetAccountsHandlerTests` — paging and search filtering
  - File: `src/CRM.Tests/Features/Accounts/GetAccountsHandlerTests.cs`

- [x] **T-28** Write `AccountListPage.test.tsx` — renders list, search, delete confirm dialog
  - File: `web/src/pages/Accounts/AccountListPage.test.tsx`

- [x] **T-29** Write `AccountFormPage.test.tsx` — validation error on empty AccountName, submit calls correct API
